...
public class KNN {
	...
	public static int[] getKNN(int n, int []req, int [][]users, int [][]vars){
		
		double [] reqs_normalized= MinMaxNormalization.normalize(req, vars);
		HashMap<Double, Integer> hmap = new HashMap<Double, Integer>();
		
		for (int i=0; i<users.length;i++){
			double [] user_normalized= MinMaxNormalization.normalize(users[i], vars);
	...
	public static int aggregateProductID (int[] similarIndexes){
